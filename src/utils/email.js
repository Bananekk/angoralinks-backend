...
// Zmień na swoją zweryfikowaną domenę!
const DEFAULT_FROM = process.env.EMAIL_FROM || 'AngoraLinks <noreply@twojadomena.pl>';
...
async function sendVerificationEmail(email, code) {
    try {
        ...
        console.log(`✅ Email weryfikacyjny wysłany do: ${email} (id: ${result.data?.id})`);
        return true;
    } catch (error) {
...
async function sendPasswordResetEmail(email, code) {
    try {
        ...
        console.log(`✅ Email resetujący wysłany do: ${email} (id: ${result.data?.id})`);
        return true;
    } catch (error) {
        console.error('❌ Błąd wysyłania emaila:', error.message);
        throw error;
...
async function sendPayoutNotification(email, amount, status, method) {
    ...
    try {
        ...
        console.log(`✅ Email o wypłacie wysłany do: ${email} (id: ${result.data?.id})`);
        return true;
    } catch (error) {
        console.error('❌ Błąd wysyłania emaila:', error.message);
        throw error;
